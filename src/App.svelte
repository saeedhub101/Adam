<script lang="ts">
  ...
  import { cloudChatStream, hasApiKey, saveApiKey, deleteApiKey, cloudCancel, cloudTestConnection, type ChatMessage } from "./lib/cloud";
  ...
  let chatOpen = $state(false); let chatInput = $state(""); let chatReply = $state(""); let cloudKey = $state(""); let cloudReady = $state(false); let chatBusy = $state(false); let chatRequestId = $state(""); let chatHistory = $state<ChatMessage[]>([]); let providerStatus = $state(""); let reminderToast = $state("");
  ...
  let settingsOpen = $state(false); let settingsTab = $state("general"); let dnd = $state(false); let startWithWindows = $state(true); let themeMode = $state("system"); let emergencyHotkey = $state("Ctrl+Alt+X"); let personalityIntensity = $state(70); let lowPowerMode = $state(false); let micMode = $state("on"); let ttsRate = $state(1); let ttsVolume = $state(1); let shareMemory = $state(false); let offlineOnly = $state(false); let telemetry = $state(false); let logRetentionDays = $state(30); let excludedAppsText = $state(""); let settingsSaved = $state(false);
  ...
  onMount(async () => {
    ...
    try { const cfg=JSON.parse(localStorage.getItem("adam-settings")||"{}"); startWithWindows=cfg.startWithWindows??true; dnd=cfg.dnd??false; themeMode=cfg.themeMode??"system"; emergencyHotkey=cfg.emergencyHotkey??"Ctrl+Alt+X"; personalityIntensity=cfg.personalityIntensity??70; lowPowerMode=cfg.lowPowerMode??false; micMode=cfg.micMode??"on"; ttsRate=cfg.ttsRate??1; ttsVolume=cfg.ttsVolume??1; shareMemory=cfg.shareMemory??false; offlineOnly=cfg.offlineOnly??false; telemetry=cfg.telemetry??false; logRetentionDays=cfg.logRetentionDays??30; excludedAppsText=cfg.excludedAppsText??""; } catch {}
  });
  ...
  async function openSettings() { settingsOpen=true; showMenu=false; clickThrough=false; await setIgnoreCursorEvents(false); permissions=await listPermissions(); activity=await listActivity(); }
  function saveSettings(){ localStorage.setItem("adam-settings",JSON.stringify({startWithWindows,dnd,themeMode,emergencyHotkey,personalityIntensity,lowPowerMode,micMode,ttsRate,ttsVolume,shareMemory,offlineOnly,telemetry,logRetentionDays,excludedAppsText})); settingsSaved=true; setTimeout(()=>settingsSaved=false,1500); }
...
<svelte:window onkeydown={(e) => { if (e.key === "Escape") { if(settingsOpen) settingsOpen=false; else void closeMenu(); } if(e.ctrlKey&&e.altKey&&e.key.toLowerCase()==="x") void stopAll(); }} />
...
<div class="stage" class:drag-over={dragOver} ondragover={(e) => { e.preventDefault(); dragOver = true; }} ondragleave={() => dragOver = false} ondrop={(e) => { e.preventDefault(); dragOver = false; const files = e.dataTransfer?.files; if (files?.length) void loadCharacterFiles(files); }} ondblclick={() => void openMenu()} oncontextmenu={(e) => { e.preventDefault(); void openMenu(); }} onpointerdown={startDrag} onpointermove={(e) => trackPointer(e)}>
  ...
  <div class="bubble">{reminderToast || t(lang, "idle")}</div>
  {#if settingsOpen}
<div class="settings-overlay" data-no-drag onpointerdown={(e)=>e.stopPropagation()}><section class="settings-window" role="dialog" aria-modal="true" aria-label="Adam Settings">
<header class="settings-header"><h2>Adam Settings</h2><button aria-label="Close" onclick={()=>settingsOpen=false}>×</button></header>
<div class="settings-grid"><nav class="settings-tabs">{#each [["general","General"],["character","Character"],["voice","Voice"],["brain","Brain"],["permissions","Permissions"],["memory","Memory & Calendar"],["privacy","Privacy"],["activity","Activity Log"],["about","About"]] as tab}<button class:active={settingsTab===tab[0]} onclick={()=>settingsTab=tab[0]}>{tab[1]}</button>{/each}</nav>
<div class="settings-content">
{#if settingsTab==="general"}<h3>General</h3><label>Start with Windows <input type="checkbox" bind:checked={startWithWindows}/></label><label>Language <select bind:value={lang} onchange={()=>setupVoice()}><option value="en">English</option><option value="ar">العربية</option></select></label><label>Emergency Stop <input bind:value={emergencyHotkey}/></label><label>Do-Not-Disturb <input type="checkbox" bind:checked={dnd}/></label><label>Theme <select bind:value={themeMode}><option value="system">Follow Windows</option><option value="light">Light</option><option value="dark">Dark</option></select></label><p>Quit Adam from tray → Close/Exit.</p>
{:else if settingsTab==="character"}<h3>Character</h3><button onclick={chooseCharacter}>Import FBX / GLB</button><label>Size {size}% <input type="range" min="60" max="160" step="5" bind:value={size} oninput={resizeAdam}/></label><label>Personality intensity {personalityIntensity}% <input type="range" min="0" max="100" bind:value={personalityIntensity}/></label><label>Low-power mode <input type="checkbox" bind:checked={lowPowerMode}/></label><div class="settings-actions"><button onclick={()=>scene?.setState("idle")}>Test Idle</button><button onclick={()=>scene?.setState("walk")}>Test Walk</button><button onclick={()=>scene?.setState("gesture")}>Test Gesture</button></div>
{:else if settingsTab==="voice"}<h3>Voice</h3><label>Mode <select bind:value={micMode}><option value="on">Mic On</option><option value="off">Mic Off</option><option value="mute">Mute</option></select></label><label>TTS speed <input type="range" min="0.5" max="2" step="0.05" bind:value={ttsRate}/></label><label>TTS volume <input type="range" min="0" max="1" step="0.05" bind:value={ttsVolume}/></label><button onclick={()=>void toggleVoice()}>Test microphone</button><button onclick={()=>void toggleLocalVoice()}>STT / Model Manager</button><button onclick={()=>speak(lang==="ar"?"مرحباً، أنا آدم.":"Hello, I am Adam.")}>Preview TTS</button>
{:else if settingsTab==="brain"}<h3>Brain</h3><label>Offline-only <input type="checkbox" bind:checked={offlineOnly}/></label><label>Provider <select bind:value={provider} onchange={selectProvider}>{#each Object.entries(providers) as [key,p]}<option value={key}>{p.name}</option>{/each}</select></label><label>Base URL <input bind:value={customBaseUrl}/></label><label>Model <input bind:value={model}/></label><label>API key <input type="password" bind:value={cloudKey} placeholder={cloudReady?"Stored securely":"Write-only"}/></label><div class="settings-actions"><button onclick={saveCloudKey}>Save key</button><button onclick={testProvider}>Test connection</button></div><label>Share relevant memory <input type="checkbox" bind:checked={shareMemory}/></label>
{:else if settingsTab==="permissions"}<h3>Permissions</h3>{#each permissions as permission}<label class="permission-row"><span>{permission.capability}</span><select value={permission.mode} onchange={(e)=>void changePermission(permission.capability,(e.currentTarget as HTMLSelectElement).value)}><option value="ask">Ask</option><option value="session">Session</option><option value="always">Always</option><option value="deny">Deny</option></select></label>{/each}<button onclick={()=>void stopAll()}>Emergency Stop</button>
{:else if settingsTab==="memory"}<h3>Memory & Calendar</h3><button onclick={async()=>{memories=await listMemories();memoryOpen=true}}>Memory / Facts</button><button onclick={async()=>{reminders=await listReminders();reminderOpen=true}}>Reminders</button><p>Calendar, notes and facts remain local.</p>
{:else if settingsTab==="privacy"}<h3>Privacy</h3><label>Excluded apps <textarea rows="4" bind:value={excludedAppsText} placeholder="One application per line"></textarea></label><label>Telemetry <input type="checkbox" bind:checked={telemetry}/></label><label>Log retention days <input type="number" min="1" max="3650" bind:value={logRetentionDays}/></label><p>Telemetry is off by default. Screenshots are not stored by default.</p>
{:else if settingsTab==="activity"}<h3>Activity Log</h3><div class="settings-log">{#each activity.slice(0,100) as item}<div><small>{item.createdAt}</small> · {item.action} · {item.detail}</div>{/each}</div>
{:else}<h3>About</h3><p>Adam 0.1.0 — Windows 10 22H2 / Windows 11 x64.</p><p>Production signing and updater verification require a configured certificate.</p>{/if}</div></div>
<footer class="settings-footer"><span>{settingsSaved?"Saved":""}</span><button onclick={saveSettings}>Save settings</button><button onclick={()=>settingsOpen=false}>Close</button></footer></section></div>{/if}
  {#if showMenu}
    <div class="menu" onpointerdown={(e) => e.stopPropagation()} onpointermove={(e) => e.stopPropagation()}>
      ...
      <button onclick={() => chatOpen = !chatOpen}>{lang === "ar" ? "محادثة الذكاء الاصطناعي" : "AI Chat"}</button><button onclick={() => void openSettings()}>Settings</button>
      {#if chatOpen}<div class="chat-panel">
          ...
          <button onclick={saveCloudConfig}>Save AI settings</button><button onclick={testProvider}>Test provider</button>{#if providerStatus}<small>{providerStatus}</small>{/if}
          {#if !cloudReady}<input type="password" placeholder="API key" bind:value={cloudKey} /><button onclick={saveCloudKey}>Save key</button>{:else}<input placeholder={lang === "ar" ? "اكتب لآدم" : "Message Adam"} bind:value={chatInput} onkeydown={(e) => e.key === "Enter" && sendChat()} /><div class="chat-actions"><button disabled={chatBusy} onclick={() => sendChat()}>{chatBusy ? "..." : "Send"}</button>{#if chatBusy}<button onclick={cancelChat}>Cancel</button>{/if}<button onclick={clearChatHistory}>Clear history</button><button onclick={async () => { await deleteApiKey(); cloudReady = false; }}>Remove key</button></div>{/if}
          {#if chatHistory.length}<div class="chat-history">{#each chatHistory.slice(-8) as message}<div><b>{message.role === "user" ? "You" : "Adam"}:</b> {message.content}</div>{/each}</div>{/if}{#if chatReply}<div class="chat-reply">{chatReply}</div>{/if}</div>{/if}
...
<style>.settings-overlay{position:fixed;inset:0;z-index:50;background:rgba(0,0,0,.5);display:flex;align-items:center;justify-content:center}.settings-window{width:min(900px,94vw);height:min(700px,92vh);background:#20242c;color:white;border-radius:16px;display:flex;flex-direction:column;box-shadow:0 20px 80px rgba(0,0,0,.5)}.settings-header,.settings-footer{display:flex;justify-content:space-between;align-items:center;padding:14px 18px;border-bottom:1px solid rgba(255,255,255,.12)}.settings-footer{border-top:1px solid rgba(255,255,255,.12);border-bottom:0}.settings-grid{display:grid;grid-template-columns:190px 1fr;min-height:0;flex:1}.settings-tabs{padding:10px;display:flex;flex-direction:column;gap:4px;border-right:1px solid rgba(255,255,255,.1);overflow:auto}.settings-tabs button{text-align:left;background:transparent;color:inherit;border:0;border-radius:8px;padding:9px;cursor:pointer}.settings-tabs button.active{background:rgba(80,120,190,.3)}.settings-content{padding:20px;overflow:auto;display:flex;flex-direction:column;gap:12px}.settings-content label{display:flex;justify-content:space-between;align-items:center;gap:12px}.settings-content input,.settings-content select,.settings-content textarea,.settings-content button{font:inherit;padding:8px;border-radius:8px;border:1px solid rgba(255,255,255,.15);background:rgba(255,255,255,.07);color:inherit}.settings-actions{display:flex;gap:8px;flex-wrap:wrap}.settings-log{max-height:480px;overflow:auto;background:rgba(0,0,0,.2);padding:8px;border-radius:8px}.settings-window button{cursor:pointer}.stage{position:relative;width:100vw;height:100vh;overflow:visible;user-select:none;touch-action:none}.stage.drag-over{outline:2px dashed rgba(100,180,255,.9);outline-offset:-4px}.error{padding:7px;border-radius:8px;background:rgba(180,40,40,.3);color:#ffd7d7}.capabilities{font-size:11px;opacity:.8}.animation-panel{display:flex;flex-direction:column;gap:5px}.loading{padding:6px;border-radius:8px;background:rgba(80,140,255,.18)}.loading.hidden{display:none}.quality-row{display:flex;justify-content:space-between;align-items:center;gap:8px}.quality-row label{display:flex;align-items:center;gap:5px}.bone-capabilities{display:grid;grid-template-columns:1fr 1fr;gap:2px;font-size:10px}.bone-capabilities span{opacity:.9}.bone-capabilities .missing{opacity:.45}.history{opacity:.65;word-break:break-word}.animation-panel small{font-size:10px;opacity:.7;word-break:break-word}.stage canvas{display:block;width:100%;height:100%}.quickbar{position:absolute;left:50%;bottom:10px;transform:translateX(-50%);display:flex;gap:6px;padding:5px;border-radius:14px;background:rgba(20,24,32,.72);backdrop-filter:blur(8px);z-index:5}.quickbar button{width:34px;height:32px;padding:0;border:0;border-radius:9px;background:rgba(55,65,82,.9);color:white;cursor:pointer}.quickbar button:hover,.quickbar button.active{background:rgba(75,110,170,.95)}.bubble{position:absolute;left:50%;bottom:8px;transform:translateX(-50%);padding:4px 10px;border-radius:12px;background:rgba(20,25,35,.72);color:white;font:12px sans-serif;pointer-events:none}.size-help{opacity:.65;font-size:10px}.mic-error{font-size:11px}.menu{position:absolute;right:8px;top:8px;width:260px;max-height:90vh;overflow:auto;padding:10px;border-radius:14px;background:rgba(20,24,32,.94);color:white;font:13px sans-serif;display:flex;flex-direction:column;gap:7px}.menu button,.menu input,.menu select,.menu textarea{font:inherit;border-radius:8px;border:1px solid rgba(255,255,255,.18);padding:7px;box-sizing:border-box}.menu button{background:#2f3746;color:white}.menu input,.menu select,.menu textarea{width:100%;background:#151a22;color:white}.chat-panel{padding:7px;border-radius:10px;background:rgba(255,255,255,.06);display:flex;flex-direction:column;gap:6px}.safety-panel{padding:7px;border-radius:10px;background:rgba(255,255,255,.06);display:flex;flex-direction:column;gap:6px} .safety-panel label{display:flex;justify-content:space-between;gap:6px} .activity-log{max-height:140px;overflow:auto;padding:6px;background:rgba(0,0,0,.18);border-radius:8px}.chat-reply{white-space:pre-wrap;max-height:180px;overflow:auto;padding:7px;border-radius:8px;background:rgba(255,255,255,.08)}.chat-history{max-height:140px;overflow:auto;font-size:11px;display:flex;flex-direction:column;gap:4px}.chat-actions{display:flex;flex-wrap:wrap;gap:5px}</style>
