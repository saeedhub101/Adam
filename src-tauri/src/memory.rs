...
pub fn update(path:&Path,id:i64,content:&str,kind:&str)->Result<(),String>{let conn=Connection::open(path).map_err(|e|e.to_string())?;conn.execute("UPDATE memories SET content=?1,kind=?2,updated_at=CURRENT_TIMESTAMP WHERE id=?3",params![content,kind,id]).map_err(|e|e.to_string())?;Ok(())}
pub fn delete(path:&Path,id:i64)->Result<(),String>{let conn=Connection::open(path).map_err(|e|e.to_string())?;conn.execute("DELETE FROM memories WHERE id=?1",params![id]).map_err(|e|e.to_string())?;Ok(())}
