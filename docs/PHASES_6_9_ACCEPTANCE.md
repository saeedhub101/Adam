...
- install, launch, uninstall and data-cleanup smoke coverage
- upgradeCode is stable for MSI; a true version-to-version upgrade is still a physical release QA item
...
- imported character assets are treated as untrusted input and validated before use (file size, renderable mesh, finite bounds, and triangle-count ceiling)
...
- final release QA must still be executed on real Windows 10 22H2 and Windows 11 machines, including DPI and multi-monitor permutations, real microphone/TTS devices, and the supplied final Adam character asset

## Definition of 100%

"100%" means all repository-verifiable requirements are implemented and green in CI; physical Windows/device/certificate acceptance is a separate required release sign-off and is not silently inferred from CI. Hardware-, OS-, certificate-, microphone-device-, and final-character-specific acceptance must be verified on the target Windows machines; CI cannot honestly substitute for those physical tests.
